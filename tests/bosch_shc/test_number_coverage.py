"""Unit tests for number.py SHCNumber — property getters and set_native_value clamping.

Covers lines not exercised by test_number_unit.py:
  - set_native_value: clamping to [native_min_value, native_max_value] (lines 69-72)
  - native_value: offset passthrough (line 76-77)
  - native_step: step_size passthrough (line 80-82)
  - native_min_value: min_offset passthrough (line 85-87)
  - native_max_value: max_offset passthrough (line 90-92)
...
# ---------------------------------------------------------------------------
# set_native_value — clamping (lines 69-72)
# ---------------------------------------------------------------------------
...
class TestSHCNumberSetNativeValue:
    def test_in_range_value_written_directly(self):
        ...
        n.set_native_value(2.0)
        assert n._device.offset == 2.0

    def test_value_above_max_clamped_to_max(self):
        """Value > max_offset must be clamped to max_offset."""
        n = _make_number(min_offset=-5.0, max_offset=5.0)
        n.set_native_value(10.0)
        assert n._device.offset == 5.0
    ...
    def test_zero_within_range_written(self):
        ...
        assert n._device.offset == 0.0

    def test_clamping_does_not_write_original_value(self):
        """When clamped, the clamped (not original) value is written to device."""
        n = _make_number(min_offset=0.0, max_offset=3.0)
        n.set_native_value(99.9)
        assert n._device.offset != 99.9
        assert n._device.offset == 3.0
